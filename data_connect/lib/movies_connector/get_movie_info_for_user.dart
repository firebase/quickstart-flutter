...
class GetMovieInfoForUserVariablesBuilder {
  ...
  GetMovieInfoForUserVariablesBuilder(
    this._dataConnect, {
    required String this.movieId,
  });
  Deserializer<GetMovieInfoForUserData> dataDeserializer =
      (dynamic json) => GetMovieInfoForUserData.fromJson(jsonDecode(json));
  Serializer<GetMovieInfoForUserVariables> varsSerializer =
      (GetMovieInfoForUserVariables vars) => jsonEncode(vars.toJson());
  Future<QueryResult<GetMovieInfoForUserData, GetMovieInfoForUserVariables>>
      execute() {
    return this.ref().execute();
  }

  QueryRef<GetMovieInfoForUserData, GetMovieInfoForUserVariables> ref() {
    GetMovieInfoForUserVariables vars = GetMovieInfoForUserVariables(
      movieId: movieId,
    );

    return _dataConnect.query(
        "GetMovieInfoForUser", dataDeserializer, varsSerializer, vars);
  }
}
...
class GetMovieInfoForUserData {
  ...
  GetMovieInfoForUserData.fromJson(dynamic json)
      : favorite_movie = json['favorite_movie'] == null
            ? null
            : GetMovieInfoForUserFavoriteMovie.fromJson(
                json['favorite_movie']) {}
